...
public class Motus_1_RawDataPacket extends DataPacket
{
    ...
    {
        if ((packet.getPacketType() != ValidPacketTypes.motus_1_raw_data_packet) ||
            (packet.getExpectedLen() != 18))
        {
	...
	public void Serialize(short[] payload) throws Exception
    {
        ...
        try
        {
            ...
            for (int i = 0; i < payload.length; i++)
            	indexToInsertElement = SerializeUtilities.BufferShortInToByteArray(payload[i], bytePayload, indexToInsertElement, Endianness.little_endian);
